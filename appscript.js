...
function saveComments(data) {
    ...
    if (!sheet) {
        ...
        const headers = ['Fecha y Hora', 'Grupo', 'Docente', 'Materias', 'Comentarios'];
        sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
...
function updateAnalysis() {
    ...
    const firstQuestionCol = headers.indexOf(QUESTIONS[0]);
    ...
    responses.forEach(row => {
        ...
        const answers = row.slice(firstQuestionCol, firstQuestionCol + QUESTIONS.length);
    ...
    Object.keys(teacherData).sort().forEach(teacher => {
        ...
        analysisSheet.getRange(currentRow, 1).setFontSize(14).setFontWeight('bold').setBackground('#fbbc04');

        analysisSheet.getRange(currentRow + 1, 1).setValue('Número de evaluaciones:');
        analysisSheet.getRange(currentRow + 1, 2).setValue(data.count);

        analysisSheet.getRange(currentRow + 2, 1).setValue('Promedio General:');
        analysisSheet.getRange(currentRow + 2, 2).setValue(overallAverage.toFixed(2));
        ...
        analysisSheet.getRange(chartStartRow, 1, 1, 2).setFontWeight('bold').setBackground('#e8eaf6');

        // Data
        QUESTIONS.forEach((question, index) => {
            analysisSheet.getRange(chartStartRow + 1 + index, 1).setValue(`P${index + 1}`);
            analysisSheet.getRange(chartStartRow + 1 + index, 2).setValue(averages[index]);
        });

        // Format numbers
        analysisSheet.getRange(chartStartRow + 1, 2, QUESTIONS.length, 1).setNumberFormat('0.00');

        // Create bar chart
        const chartRange = analysisSheet.getRange(chartStartRow, 1, QUESTIONS.length + 1, 2);
        ...
        const radarChart = analysisSheet.newChart()
        ...
        const distributionData = { "Siempre": 0, "Casi siempre": 0, "Casi nunca": 0, "Nunca": 0 };
        ...
        analysisSheet.getRange(pieChartStartRow, pieChartStartCol).setValue('Respuesta');
        analysisSheet.getRange(pieChartStartRow, pieChartStartCol + 1).setValue('Cantidad');
        analysisSheet.getRange(pieChartStartRow, pieChartStartCol, 1, 2)
            .setFontWeight('bold')
        ...
        pieData.forEach((row, index) => {
            analysisSheet.getRange(pieChartStartRow + 1 + index, pieChartStartCol).setValue(row[0]);
            analysisSheet.getRange(pieChartStartRow + 1 + index, pieChartStartCol + 1).setValue(row[1]);
        });

        // Create pie chart
        const pieChartRange = analysisSheet.getRange(pieChartStartRow, pieChartStartCol, 5, 2);
        ...
        analysisSheet.getRange(perfChartStartRow, perfChartStartCol).setValue('Nivel');
        analysisSheet.getRange(perfChartStartRow, perfChartStartCol + 1).setValue('Preguntas');
        analysisSheet.getRange(perfChartStartRow, perfChartStartCol, 1, 2)
            .setFontWeight('bold')
            .setBackground('#e8eaf6');
        ...
        perfData.forEach((row, index) => {
            analysisSheet.getRange(perfChartStartRow + 1 + index, perfChartStartCol).setValue(row[0]);
            analysisSheet.getRange(perfChartStartRow + 1 + index, perfChartStartCol + 1).setValue(row[1]);
        });

        // Create performance level pie chart (donut style)
        const perfChartRange = analysisSheet.getRange(perfChartStartRow, perfChartStartCol, 5, 2);
        const perfPieChart = analysisSheet.newChart()
            .setChartType(Charts.ChartType.PIE)
    ...
    analysisSheet.getRange(1, 1).setFontSize(16).setFontWeight('bold').setBackground('#34a853').setFontColor('#ffffff');
...
function initialSetup() {
    ...
    SpreadsheetApp.getUi().alert('Setup completo', 'Las hojas han sido creadas exitosamente.', SpreadsheetApp.getUi().ButtonSet.OK);
}